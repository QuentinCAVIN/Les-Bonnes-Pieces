...
export function ajoutListenerAvis() {
  ...
  //On ajoute les avis sur chaque pièce
  for (let i = 0; i < piecesElements.length; i++) {
    piecesElements[i].addEventListener("click", async function (event) {
      ...
      //Pour chaque commentaire, on crée et rattache à la pièce, un <p> qui contient le commentaire
...
export async function afficherGraphiqueDisponibilite() {
  ...
  for (let piece of pieces) {
    ...
    const avis = await fetch(
      `http://localhost:8081/pieces/${piece.id}/avis`
    ).then((avis) => avis.json());
