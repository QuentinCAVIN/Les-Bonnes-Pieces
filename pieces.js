...
function genererPieces(pieces) {
  for (let i = 0; i < pieces.length; i++) {
    ...
    disponibiliteElement.innerText = article.disponibilité
      ? "En stock"
      : "Rupture de stock";
...
const boutonFiltrerNonAbordable = document.querySelector(
  ".btn-filtrer-non-abordable"
);
...
const boutonFiltrerSansDescription = document.querySelector(
...
const nomsEtPrixDisponibles = pieces.map(
  (piece) => `${piece.nom} - ${piece.prix}€.`
);
for (let i = pieces.length - 1; i >= 0; i--) {
